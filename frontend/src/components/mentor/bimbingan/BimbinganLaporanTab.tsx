...
interface BimbinganLaporanTabProps {
  ...
  onUpdateStatus: (laporanId: string, status: 'disetujui' | 'ditolak') => void;
}
...
const STATUS_BADGE: Record<LaporanStatus, { label: string; className: string }> = {
  ...
  disetujui: { label: 'Disetujui', className: 'bg-emerald-50 text-emerald-700 border-emerald-300' },
                ...
                <tr key={laporan.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="py-5 px-6">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        type="button"
                        onClick={() => onUpdateStatus(laporan.id, 'disetujui')}
                        disabled={busy || laporan.status === 'disetujui'}
                      ...
                      <button
                        type="button"
                        onClick={() => onUpdateStatus(laporan.id, 'ditolak')}
